...
def registerUser(request):
    if request.user.is_authenticated:
        messages.warning(request, 'You are already log in!')
        return redirect('dashboard')
    elif request.method == 'POST':
        form = UserForm(request.POST)
        if form.is_valid():
            #: Form,
            # 'v_formcreate the user using the form: 
            ...
            #create a user using create_user methos
...
def login(request):
    if request.user.is_authenticated:
        ...
        email = request.POST['email'] #igual que el nombre de login.html
...
def reset_password(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.is_active = True
            user.save()
            messages.success(request, 'password reset succssful')
            return redirect('login')
        else:
